...
from app.structured.common import search_patterns
...
def build_1099_nec_document(raw_payload: ExtractionPayload, *, mask_pii: bool = True) -> StructuredDocument:
    ...
    recipient_name = _extract_value(text, [r"recipient(?:'s)?\s+name\s*[:#-]?\s*([^\n]+)"])
    recipient_tin_source = _extract_value(
        text,
        [
    ...
    payer_name = _extract_value(text, [r"payer(?:'s)?\s+name\s*[:#-]?\s*([^\n]+)"])
    payer_tin = _extract_value(
        text,
        [
            r"payer(?:'s)?\s+tin\s*[:#-]?\s*([^\n]+)",
    ...
    box1_text = _extract_value(text, [r"\b1\s+nonemployee compensation\s+([\$\d,().-]+)"])
    box4_text = _extract_value(text, [r"\b4\s+federal income tax withheld\s+([\$\d,().-]+)"])
